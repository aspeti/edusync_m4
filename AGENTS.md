...
| **PROMPT_MAPPING** | `docs/PROMPT_MAPPING.md` | Catálogo de prompt-contratos `PR-<AREA>-NNN` — v2.23 (48 contratos activos: PR-ARCH..PR-POC-002, PR-C4-003..006, PR-ROADMAP-001, PR-APORTES-001, PR-VFINAL-001, PR-IMPL-001..011; `PR-IMPL-001`..`011` **todos ejecutados**, incl. `PR-IMPL-008`/`PR-IMPL-009` — módulo `academico`, `GestionEscolar` backend + UI Angular — y `PR-IMPL-010`/`PR-IMPL-011` — módulo `academico`, `Curso`/`Paralelo` backend + UI Angular) |
...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.23 (21/08/2026) — continuación viva del DTI congelado; §A.3 marca `FSD-UC-011`, `FSD-UC-012`, `FSD-UC-017` y `FSD-UC-021` como **completos** (backend + UI, incl. filtros/paginación); `PR-IMPL-007`..`PR-IMPL-011` **ejecutados** (134/134 tests backend, `ng build` verde) |
| **PRD/FSD/BRD vivos** | `docs/product/{BRD,PRD,FSD}.md` | Copias editables de la capa viva (banner "COPIA VIVA"), abiertas para `release/3.0.0`; el FSD vivo opera en modo LFSD ⚡ (`docs/product/FSD.md` v2.6). Desde `ADR-0009` (v3.1/v2.2/v2.2 resp.): plataforma SaaS multi-tenant configurable (SysAdmin, Tenant con suscripción, Gestión Escolar, periodos/secciones/tipos de evaluación configurables, Cursos/Paralelos, Materias, Profesores, Estudiantes, Inscripciones, Usuarios y Roles) añadida como extensión aditiva sobre el Perfil Bolivia SIE (BR-001..BR-012/RB-01..RB-11, vigentes sin cambios). Desde `ADR-0010`: `BR-024`/`FSD-UC-021` usan un modelo multi-rol (`UsuarioRol` N:M) con la invariante permanente `tenant_id IS NULL ⟺ roles = {SYSADMIN}` |
...
├── README.md
├── AGENTS.md                        ← este archivo (v0.34) — convención multi-agente (Cursor + Claude)
...
│                                      — 48 contratos en total contando PR-IMPL-001..011 en docs/prompts/impl/
...
│                                      academico/Curso-Paralelo (PR-IMPL-010) —
│                                      monolito modular Spring Boot 4.1.0 (Java 25 LTS) spring-modulith-bom 2.1.0,
│                                      com.edusync; 134/134 tests verde (incluye ModularityTests 7/7). Los 10 prompts
│                                      IMPL de release/3.0.0 ya ejecutados (reemplaza al antiguo
...
│       ├── academico/               ← ✅ real (PR-IMPL-008 + PR-IMPL-010) — GestionEscolar (Aggregate Root, alta/listado
...
│       │                              resto (PeriodoEvaluacion..Materia/Inscripcion, FSD-UC-013..016/018..020) vacío
...
├── frontend/                        ← ✅ SPA Angular 21.2.19 LTS (PR-IMPL-001 esqueleto + PR-IMPL-004/006/009/011 UI +
...
│       └── shared/layout/           ← shell mínimo (nav condicional por rol, DD-UC-006/009/011)
