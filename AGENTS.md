...
| **PROMPT_MAPPING** | `docs/PROMPT_MAPPING.md` | Catálogo de prompt-contratos `PR-<AREA>-NNN` — v2.27 (50 contratos activos: PR-ARCH..PR-POC-002, PR-C4-003..006, PR-ROADMAP-001, PR-APORTES-001, PR-VFINAL-001, PR-IMPL-001..013; `PR-IMPL-001`..`013` **ejecutados**, incl. `PR-IMPL-013` — módulo `academico`, `Estudiante`/`Inscripcion` backend + UI fullstack, `FSD-UC-020`) |
...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.27 (21/08/2026) — continuación viva del DTI congelado; §A.3 marca `FSD-UC-011`, `FSD-UC-012`, `FSD-UC-017`, `FSD-UC-018`, `FSD-UC-020` y `FSD-UC-021` como **completos** (backend + UI); `PR-IMPL-007`..`PR-IMPL-013` **ejecutados** (173/173 tests backend, `ng build` verde) |
| **PRD/FSD/BRD vivos** | `docs/product/{BRD,PRD,FSD}.md` | Copias editables de la capa viva (banner "COPIA VIVA"), abiertas para `release/3.0.0`; el FSD vivo opera en modo LFSD ⚡ (`docs/product/FSD.md` v2.8). Desde `ADR-0009` (v3.1/v2.2/v2.2 resp.): plataforma SaaS multi-tenant configurable (SysAdmin, Tenant con suscripción, Gestión Escolar, periodos/secciones/tipos de evaluación configurables, Cursos/Paralelos, Materias, Profesores, Estudiantes, Inscripciones, Usuarios y Roles) añadida como extensión aditiva sobre el Perfil Bolivia SIE (BR-001..BR-012/RB-01..RB-11, vigentes sin cambios). Desde `ADR-0010`: `BR-024`/`FSD-UC-021` usan un modelo multi-rol (`UsuarioRol` N:M) con la invariante permanente `tenant_id IS NULL ⟺ roles = {SYSADMIN}` |
...
├── README.md
├── AGENTS.md                        ← este archivo (v0.38) — convención multi-agente (Cursor + Claude)
...
│                                      — 50 contratos en total contando PR-IMPL-001..013 en docs/prompts/impl/
...
│                                      com.edusync; 173/173 tests verde (incluye ModularityTests 7/7). PR-IMPL-001..013
│                                      ejecutados
...
│       ├── academico/               ← ✅ real (PR-IMPL-008 + PR-IMPL-010 + PR-IMPL-012 + PR-IMPL-013) — GestionEscolar (Aggregate Root, alta/listado
...
│       │                              resto (PeriodoEvaluacion..Evaluacion, FSD-UC-013..016/019) vacío
...
├── frontend/                        ← ✅ SPA Angular 21.2.19 LTS (PR-IMPL-001 esqueleto + PR-IMPL-004/006/009/011/012/013 UI +
...
│       └── shared/layout/           ← shell mínimo (nav condicional por rol, DD-UC-006/009/011/012/013)
