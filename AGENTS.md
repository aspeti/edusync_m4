...
| **PROMPT_MAPPING** | `docs/PROMPT_MAPPING.md` | Catálogo de prompt-contratos `PR-<AREA>-NNN` — v2.15 (45 contratos activos: PR-ARCH..PR-POC-002, PR-C4-003..006, PR-ROADMAP-001, PR-APORTES-001, PR-VFINAL-001, PR-IMPL-001..008; los ocho `PR-IMPL-*` **ejecutados**, incl. `PR-IMPL-008` — primer feature de negocio real del módulo `academico`, `GestionEscolar`) |
...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.16 (20/08/2026) — continuación viva del DTI congelado; §A.3 marca `FSD-UC-011` y `FSD-UC-021` como **completos** (backend + UI, incl. filtros/paginación) y `FSD-UC-012` como **completo (backend)**, UI pendiente; `PR-IMPL-007`/`PR-IMPL-008` **ejecutados** (119/119 tests backend, `ng build` verde) |
...
├── README.md
├── AGENTS.md                        ← este archivo (v0.29) — convención multi-agente (Cursor + Claude)
...
│                                      — 45 contratos en total contando PR-IMPL-001..008 en docs/prompts/impl/
...
│                                      com.edusync; 119/119 tests verde (incluye ModularityTests 7/7). Los 8 prompts
...
│       ├── academico/               ← ✅ real (PR-IMPL-008) — GestionEscolar (Aggregate Root, alta/listado con
│       │                              filtros-paginación/ciclo de estado PLANIFICACION-ACTIVA-CERRADA),
│       │                              GestionEscolarSpecifications (FSD-UC-012 completo backend, DD-UC-008;
│       │                              UI pendiente); resto (PeriodoEvaluacion..Inscripcion, FSD-UC-013..020) vacío
...
├── frontend/                        ← ✅ SPA Angular 21.2.19 LTS (PR-IMPL-001 esqueleto + PR-IMPL-004/006 UI +
│                                      PR-IMPL-007 filtros/paginación, 20/08/2026) (standalone, sin Nx); ng build sin errores
│   └── src/app/
...
│       └── shared/layout/           ← shell mínimo (nav condicional por rol, DD-UC-006)
