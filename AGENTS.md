...
| **PROMPT_MAPPING** | `docs/PROMPT_MAPPING.md` | Catálogo de prompt-contratos `PR-<AREA>-NNN` — v2.4 (40 contratos activos: PR-ARCH..PR-POC-002, PR-C4-003..006, PR-ROADMAP-001, PR-APORTES-001, PR-VFINAL-001, PR-IMPL-001, PR-IMPL-002, PR-IMPL-003; v2.0 abrió el área `IMPL` para `release/3.0.0`, v2.1 materializó su primera fila `PR-IMPL-001` (bootstrap), v2.2 corrigió la ruta a `docs/prompts/impl/`, v2.3 añadió `PR-IMPL-002` (módulo `identidad`/login), v2.4 añade `PR-IMPL-003` — módulo `plataforma`/alta de Tenants) |
...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.6 (14/07/2026) — continuación viva del DTI congelado, punto de partida para `release/3.0.0`; registra el delta de stack `ADR-0008`, el delta de generalización del modelo de dominio `ADR-0009`, el delta de refinamiento del modelo de roles (multi-rol + `SysAdmin` sin tenant permanente) `ADR-0010` y el delta de organización interna del backend (monolito modular Spring Modulith + paquete `com.edusync`) `ADR-0011` en §A.2; §A.3 marca `FSD-UC-011` como `en progreso` con `DD-UC-003` (alta de Tenants, completo) y `FSD-UC-021` como `en progreso` con `DD-UC-002` (módulo `identidad`/login); el resto de FSD-UC sigue `pendiente` |
...
| **Design Docs (capa viva)** | `docs/design/DD-UC-NNN.md` | Documentos de diseño por feature/*vertical slice*, trazados a `FSD-UC` + `ADR` + `PR-IMPL-NNN`; alimentan el DTP vía `@dtp-sync`. `DD-UC-001` (bootstrap del proyecto — `FSD-UC-011`/`FSD-UC-021`, crea `ADR-0011`); `DD-UC-002` (módulo `identidad` — `Usuario`/`UsuarioRol`, login JWT, seed `SYSADMIN`, `TenantContextProvider` real — `FSD-UC-021` parcial, consume `ADR-0001`/`ADR-0010`/`ADR-0011`); `DD-UC-003` (módulo `plataforma` — `Tenant`, ciclo de suscripción, scheduler de vencimiento, `TenantConsultaPort` — `FSD-UC-011` completo, consume `ADR-0009`/`ADR-0010`/`ADR-0011` y `UsuarioCreacionPort` de `DD-UC-002`) |
...
├── README.md
├── AGENTS.md                        ← este archivo (v0.20) — convención GitHub/Cursor, raíz requerida por la rúbrica del Módulo 4
...
│   ├── PROMPT_MAPPING.md            ← catálogo de 40 prompt-contratos v2.4 (área IMPL: 3 filas, PR-IMPL-001..003)
...
│   │   └── DTP.md                   ← Documento Técnico del Producto v1.6 (ADR-0008/0009/0010/0011 registrados en §A.2)
...
│   │   ├── PR-IMPL-001.md           ← bootstrap del esqueleto backend/frontend/infra (aprobado, ejecución pendiente)
│   │   ├── PR-IMPL-002.md           ← módulo identidad: login/JWT + seed SysAdmin (aprobado, ejecución pendiente)
│   │   └── PR-IMPL-003.md           ← módulo plataforma: alta de Tenants + TenantConsultaPort (aprobado, ejecución pendiente)
│   ├── brd/
...
│                                      — 39 contratos en total contando PR-IMPL-001/002 en docs/prompts/impl/
├── backend/                         ← ✅ esqueleto generado (PR-IMPL-001, 18/07/2026) — monolito modular
│                                      Spring Boot 4.1.0 (Java 25 LTS) sobre spring-modulith-bom 2.1.0, paquete base
│                                      com.edusync; ModularityTests 7/7 verde. Logica de dominio real (Usuario/Tenant)
│                                      pendiente de PR-IMPL-002/003 (reemplaza al antiguo src/domain|application|infrastructure)
│   ├── pom.xml
│   └── src/main/java/com/edusync/
│       ├── EduSyncApplication.java
│       ├── plataforma/              ← vacio (package-info.java) — SysAdmin, Tenant, Suscripcion (FSD-UC-011, DD-UC-003)
│       ├── identidad/               ← vacio (package-info.java) — Usuario, UsuarioRol, login/JWT/RBAC (FSD-UC-021, DD-UC-002)
...
│       └── shared/                  ← TenantContext (placeholder), DomainException (base); modulo OPEN de Spring Modulith
...
- [x] Sincronizado con `docs/PROMPT_MAPPING.md` v2.4 (40 prompt-contratos + carpeta `prompts/`; área `IMPL` con 3 filas `PR-IMPL-001..003`, las tres en `docs/prompts/impl/`).
...
- [ ] Ejecución de `PR-IMPL-002` y `PR-IMPL-003` (lógica de dominio real de `identidad`/`plataforma`) — siguiente paso concreto de implementación.
