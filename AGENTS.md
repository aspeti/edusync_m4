...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.8 (19/07/2026) — continuación viva del DTI congelado, punto de partida para `release/3.0.0`; registra el delta de stack `ADR-0008`, el delta de generalización del modelo de dominio `ADR-0009`, el delta de refinamiento del modelo de roles (multi-rol + `SysAdmin` sin tenant permanente) `ADR-0010`, el delta de organización interna del backend (monolito modular Spring Modulith + paquete `com.edusync`) `ADR-0011` y el delta de herramientas de productividad backend (Lombok con *allowlist* en `domain/`, springdoc-openapi, Bean Validation) `ADR-0012` en §A.2; §A.3 marca `FSD-UC-011` como `en progreso` con `DD-UC-003` (alta de Tenants, solo bootstrap) y `FSD-UC-021` como `en progreso` con `DD-UC-002` (módulo `identidad`/login, **ejecutado y con 27/27 tests verdes**); el resto de FSD-UC sigue `pendiente` |
...
├── README.md
├── AGENTS.md                        ← este archivo (v0.21) — convención GitHub/Cursor, raíz requerida por la rúbrica del Módulo 4
...
│   │   └── DTP.md                   ← Documento Técnico del Producto v1.8 (ADR-0008/0009/0010/0011/0012 registrados en §A.2)
...
│                                      (PR-IMPL-002 + ADR-0012, 18-19/07/2026) — monolito modular Spring Boot 4.1.0
│                                      (Java 25 LTS) sobre spring-modulith-bom 2.1.0, paquete base com.edusync;
│                                      27/27 tests verde (incluye ModularityTests 7/7). Logica de dominio real de
│                                      plataforma (Tenant) pendiente de PR-IMPL-003 (reemplaza al antiguo
...
- [ ] Ejecución de `PR-IMPL-003` (lógica de dominio real de `plataforma`) — siguiente paso concreto de implementación.
