...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.0 (28/05/2026) — continuación viva del DTI congelado, punto de partida para `release/3.0.0`; registra el delta de stack `ADR-0008` en §A.2; §A.3 lista los 5 FSD-UC del baseline como `pendiente` (sin `DD-UC-NNN` ni `PR-IMPL-NNN` todavía) |
| **PRD/FSD/BRD vivos** | `docs/product/{BRD,PRD,FSD}.md` | Copias editables de la capa viva (banner "COPIA VIVA"), abiertas para `release/3.0.0`; el FSD vivo opera en modo LFSD ⚡ |
...
| **ADRs** | `docs/adr/0001..0006-*.md`, `0008-*.md` | 7 ADRs aprobados: 0001 multitenancy RLS, 0002 parametrización reglas normativas, 0003 audit_log append-only, 0004 async consolidación (Spring Events), 0005 resiliencia SIE (Resilience4j), 0006 cloud provider AWS, 0008 stack vivo Java 25 LTS/Spring Boot 4.1.0/Angular 21 LTS. `ADR-0007` (Strangler Fig) queda *gated*, sin crear todavía (ver `docs/roadmap.md` §4) |
...
- [x] `docs/baseline/` protegido — 5 archivos marcados `status: congelado`, `CODEOWNERS` y `.cursor/rules/baseline-congelado.mdc` creados.
