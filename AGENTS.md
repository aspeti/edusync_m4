...
| **PROMPT_MAPPING** | `docs/PROMPT_MAPPING.md` | Catálogo de prompt-contratos `PR-<AREA>-NNN` — v2.38 (56 contratos activos: PR-ARCH..PR-POC-002, PR-C4-003..006, PR-ROADMAP-001, PR-APORTES-001, PR-VFINAL-001, PR-ADR-006, PR-IMPL-001..018; `PR-IMPL-001`..`018` **ejecutados**; `PR-ADR-006` = `ADR-0013`) |
...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.38 (21/08/2026) — continuación viva del DTI congelado; §A.3 marca `FSD-UC-011`, `FSD-UC-012`, `FSD-UC-013`, `FSD-UC-014`, `FSD-UC-015`, `FSD-UC-016`, `FSD-UC-017`, `FSD-UC-018`, `FSD-UC-019`, `FSD-UC-020` y `FSD-UC-021` como **completos** (backend + UI); `PR-IMPL-007`..`PR-IMPL-018` **ejecutados** (235/235 tests backend, `ng build` verde) |
| **PRD/FSD/BRD vivos** | `docs/product/{BRD,PRD,FSD}.md` | Copias editables de la capa viva (banner "COPIA VIVA"), abiertas para `release/3.0.0`; el FSD vivo opera en modo LFSD ⚡ (`docs/product/FSD.md` v2.14, BRD v3.2, PRD v2.3). Desde `ADR-0009`/`ADR-0013`: plataforma SaaS multi-tenant configurable; periodos/secciones/cálculo genérico resueltos (puntos 1–4 de `ADR-0009` §3). Desde `ADR-0010`: `BR-024`/`FSD-UC-021` multi-rol. |
...
├── README.md
├── AGENTS.md                        ← este archivo (v0.47) — convención multi-agente (Cursor + Claude)
...
│   │   └── DD-UC-017.md             ← Académico: Evaluaciones backend+UI fullstack (ejecutado, 21/08/2026; FSD-UC-015 completo)
│   │   └── DD-UC-018.md             ← Académico: Calificaciones + CalculoNotas backend+UI fullstack (ejecutado, 21/08/2026; FSD-UC-016 completo)
...
│   │   └── PR-IMPL-017.md           ← academico: Evaluaciones (backend+UI fullstack, ejecutado 21/08/2026)
│   │   └── PR-IMPL-018.md           ← academico: Calificaciones + CalculoNotas (backend+UI fullstack, ejecutado 21/08/2026)
│   ├── brd/
...
│                                      academico/Evaluacion (PR-IMPL-017) +
│                                      academico/CalificacionEvaluacion (PR-IMPL-018) —
│                                      monolito modular Spring Boot 4.1.0 (Java 25 LTS) spring-modulith-bom 2.1.0,
│                                      com.edusync; 235/235 tests verde (incluye ModularityTests 7/7). PR-IMPL-001..018
│                                      ejecutados (FSD-UC-016 CalificacionEvaluacion + CalculoNotas completo)
...
│       ├── academico/               ← ✅ real (PR-IMPL-008 + PR-IMPL-010 + PR-IMPL-012 + PR-IMPL-013 + PR-IMPL-014 + PR-IMPL-015 + PR-IMPL-016 + PR-IMPL-017 + PR-IMPL-018) — GestionEscolar (Aggregate Root, alta/listado
...
│       │                              PeriodoEvaluacion (FSD-UC-013 completo backend+UI, DD-UC-015); SeccionEvaluacion (FSD-UC-014 completo backend+UI, DD-UC-016); Evaluacion (FSD-UC-015 completo backend+UI, DD-UC-017); CalificacionEvaluacion + CalculoNotas (FSD-UC-016 completo backend+UI, DD-UC-018))
...
├── frontend/                        ← ✅ SPA Angular 21.2.19 LTS (PR-IMPL-001 esqueleto + PR-IMPL-004/006/009/011/012/013/014/015/016/017 UI +
...
│       │                              Evaluaciones (Mis materias + detalle por materia, DD-UC-017; PROFESOR+ADMIN);
│       │                              Calificaciones (matriz por evaluación + provisional, DD-UC-018; PROFESOR+ADMIN; ejecutado)
...
| v0.46 | 21/08/2026 | Rodrigo Aspeti | **Decimoséptimo Design Doc + `PR-IMPL-017`** — diseño aprobado, ejecución pendiente: sexto *vertical slice* fullstack de `academico` (`docs/design/DD-UC-017.md`) — Evaluaciones (`FSD-UC-015`). Decisiones explícitas: Aggregate `Evaluacion` independiente; `puntajeMaximo` derivado de `seccion.nota`; A1 `409 E_MATERIA_SIN_PROFESOR`; primera consola `PROFESOR` (Mis materias); `Calificacion`/`round` diferidos a `FSD-UC-016`. `docs/prompts/impl/PR-IMPL-017.md` v0.1. `docs/PROMPT_MAPPING.md` v2.34→v2.35 (55 contratos); `docs/product/DTP.md` v1.34→v1.35. Sin ADR nuevo ni código. Baseline intacto. |
| v0.47 | 21/08/2026 | Rodrigo Aspeti | **Ejecución real de `PR-IMPL-017` + sync documental (`dtp-sync`)**: código fullstack de Evaluaciones — Aggregate `Evaluacion`, `puntajeMaximo` derivado, `V11`, A1, consola `PROFESOR` (Mis materias). Verificación: `mvn test` → **228/228** verde (incluye `ModularityTests` 7/7); `ng build` verde. `docs/product/FSD.md` v2.12→v2.13; `docs/PROMPT_MAPPING.md` v2.35→v2.36; `docs/product/DTP.md` v1.35→v1.36. `FSD-UC-015` cierra implementación **completa** (backend + UI) — noveno `FSD-UC` en cerrar ambas capas. Sin ADR nuevo. Baseline intacto. |
...
- [x] Resolución de los puntos 1–4 de `ADR-0009` §3 vía `ADR-0013` (21/08/2026): modelo genérico único, secuencialidad, `round` genérico, suma de secciones = 100. **Sigue pendiente** el punto 5 (gobernanza de módulos nuevos). `FSD-UC-013`, `FSD-UC-014`, `FSD-UC-015` y `FSD-UC-016` **completos** (backend+UI).
...
- [x] Decimoséptimo Design Doc `DD-UC-017` + `PR-IMPL-017` (21/08/2026): módulo `academico` — Evaluaciones (backend + UI fullstack en un solo DD) — **aprobados y ejecutados**. Sexto *vertical slice* fullstack de `academico`. Decisiones explícitas: Aggregate independiente; `puntajeMaximo` derivado; A1 `E_MATERIA_SIN_PROFESOR`; primera consola `PROFESOR`.
- [x] Ejecución de `PR-IMPL-017` (21/08/2026): noveno `FSD-UC` en cerrar backend+UI (`FSD-UC-015`): `Evaluacion`, `V11`, UI Mis materias / evaluaciones. `mvn test` → **228/228** verde (incluye `ModularityTests` 7/7); `ng build` verde. Docs sincronizados (`DTP` v1.36, `PROMPT_MAPPING` v2.36, `FSD` v2.13). Commit formal pendiente.
- [x] Decimoctavo Design Doc `DD-UC-018` + `PR-IMPL-018` (21/08/2026): módulo `academico` — Calificaciones + `CalculoNotas` (backend + UI fullstack en un solo DD) — **aprobados y ejecutados**. Séptimo *vertical slice* fullstack de `academico`. Decisiones explícitas: Aggregate `CalificacionEvaluacion`; motor `round` HALF_UP sin `floor()`; matriz UI; A2 `E_RANGO_INVALIDO`.
- [x] Ejecución de `PR-IMPL-018` (21/08/2026): décimo `FSD-UC` en cerrar backend+UI (`FSD-UC-016`): `CalificacionEvaluacion`, `V12`, UI matriz calificaciones. `mvn test` → **235/235** verde (incluye `ModularityTests` 7/7); `ng build` verde. Docs sincronizados (`DTP` v1.38, `PROMPT_MAPPING` v2.38, `FSD` v2.14). Commit formal pendiente.
