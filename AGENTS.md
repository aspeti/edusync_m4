...
| **PROMPT_MAPPING** | `docs/PROMPT_MAPPING.md` | Catálogo de prompt-contratos `PR-<AREA>-NNN` — v2.3 (39 contratos activos: PR-ARCH..PR-POC-002, PR-C4-003..006, PR-ROADMAP-001, PR-APORTES-001, PR-VFINAL-001, PR-IMPL-001, PR-IMPL-002; v2.0 abrió el área `IMPL` para `release/3.0.0`, v2.1 materializó su primera fila `PR-IMPL-001` (bootstrap), v2.2 corrigió la ruta a `docs/prompts/impl/`, v2.3 añade `PR-IMPL-002` — módulo `identidad`/login) |
...
| **DTP (capa viva)** | `docs/product/DTP.md` | Documento Técnico del Producto v1.5 (14/07/2026) — continuación viva del DTI congelado, punto de partida para `release/3.0.0`; registra el delta de stack `ADR-0008`, el delta de generalización del modelo de dominio `ADR-0009`, el delta de refinamiento del modelo de roles (multi-rol + `SysAdmin` sin tenant permanente) `ADR-0010` y el delta de organización interna del backend (monolito modular Spring Modulith + paquete `com.edusync`) `ADR-0011` en §A.2; §A.3 marca `FSD-UC-011` como `en progreso` con `DD-UC-001` (bootstrap) y `FSD-UC-021` como `en progreso` con `DD-UC-002` (módulo `identidad`/login); el resto de FSD-UC sigue `pendiente` |
...
| **Design Docs (capa viva)** | `docs/design/DD-UC-NNN.md` | Documentos de diseño por feature/*vertical slice*, trazados a `FSD-UC` + `ADR` + `PR-IMPL-NNN`; alimentan el DTP vía `@dtp-sync`. `DD-UC-001` (bootstrap del proyecto — `FSD-UC-011`/`FSD-UC-021`, crea `ADR-0011`); `DD-UC-002` (módulo `identidad` — `Usuario`/`UsuarioRol`, login JWT, seed `SYSADMIN`, `TenantContextProvider` real — `FSD-UC-021` parcial, consume `ADR-0001`/`ADR-0010`/`ADR-0011`) |
...
├── README.md
├── AGENTS.md                        ← este archivo (v0.18) — convención GitHub/Cursor, raíz requerida por la rúbrica del Módulo 4
...
- [ ] Ejecución de `PR-IMPL-001` y `PR-IMPL-002` (generación real del código en `backend/`, `frontend/`, `infra/`) — `src/` sigue vacío; siguiente paso concreto de implementación.
