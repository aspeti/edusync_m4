...
> IDs: `ARCH` / `BRD` / `MRD` / `PRD` / `FSD` / `LFSD` / `UC` / `ADR` / `AUD` / `INF` / `DIAG` / `SKILL` / `C4` / `DTI` / `HEX` / `DTO` / `POC` / `ROADMAP` / `APORTES` / `VFINAL` / `IMPL`. Versión activa: `v2.44`.
...
| PR-IMPL-015 | `docs/design/DD-UC-015.md` + `docs/prompts/impl/PR-IMPL-015.md` — módulo `academico`: `PeriodoEvaluacion` (backend + consola Angular fullstack) | generación | `dev-agent` | Sonnet | 21/08/2026 | Ejecutado | `docs/prompts/impl/PR-IMPL-015.md` | `mvn test` 200/200; `ng build` verde \| `FSD-UC-013` completo backend+UI |
| PR-IMPL-016 | `docs/design/DD-UC-016.md` + `docs/prompts/impl/PR-IMPL-016.md` — módulo `academico`: `SeccionEvaluacion` (backend + consola Angular fullstack) | generación | `dev-agent` | Sonnet | 21/08/2026 | Ejecutado | `docs/prompts/impl/PR-IMPL-016.md` | `mvn test` 215/215; `ng build` verde \| `FSD-UC-014` completo |
| PR-IMPL-017 | `docs/design/DD-UC-017.md` + `docs/prompts/impl/PR-IMPL-017.md` — módulo `academico`: `Evaluacion` (backend + consola Angular fullstack) | generación | `dev-agent` | Sonnet | 21/08/2026 | Ejecutado | `docs/prompts/impl/PR-IMPL-017.md` | `mvn test` 228/228; `ng build` verde; `FSD-UC-015` completo |
| PR-IMPL-018 | `docs/design/DD-UC-018.md` + `docs/prompts/impl/PR-IMPL-018.md` — módulo `academico`: `CalificacionEvaluacion` + motor `CalculoNotas` (backend + consola Angular fullstack) | generación | `dev-agent` | Sonnet | 21/08/2026 | Ejecutado | `docs/prompts/impl/PR-IMPL-018.md` | `FSD-UC-016` completo backend+UI; `mvn test` 235/235; `ng build` verde |
...
| PR-IMPL-022 | `docs/design/DD-UC-022.md` + `docs/prompts/impl/PR-IMPL-022.md` — backend: Spring AI como cliente HTTP/LLM detrás de `LlmPort` (crea `ADR-0017`), sin cambio de contrato `POST /api/v1/ai/chat` | generación + refactor | `dev-agent` | Sonnet | 13/09/2026 | **Ejecutado** | `docs/prompts/impl/PR-IMPL-022.md` | BOM `2.0.0` vs Boot 4.1.0; `mvn test` 250/250 (incluye `ModularityTests` 7/7). `PR-IMPL-021` reservado por `DD-UC-021` |
| PR-ADR-010 | `docs/adr/0017-adopcion-spring-ai-cliente-llm.md` — Spring AI solo en `infrastructure`, detrás de `LlmPort` | generación | `arch-agent` | Sonnet | 13/09/2026 | Aprobado | `prompts/PR-ADR-010.md` | ADR formal; habilita `DD-UC-022`; `PR-ADR-009` reservado por `DD-UC-021`/`ADR-0016` |
...
| `dev-agent` | PR-UC-001..UC-010, PR-DTO-001, PR-IMPL-001, PR-IMPL-002, PR-IMPL-003, PR-IMPL-004, PR-IMPL-005, PR-IMPL-006, PR-IMPL-007, PR-IMPL-008, PR-IMPL-009, PR-IMPL-010, PR-IMPL-011, PR-IMPL-012, PR-IMPL-013, PR-IMPL-014, PR-IMPL-015, PR-IMPL-016, PR-IMPL-017, PR-IMPL-018, PR-IMPL-019, PR-IMPL-020, PR-IMPL-022 | Generar contratos de UC, DTOs por capa hexagonal, código de dominio y pruebas unitarias; desde `release/3.0.0`, materializar los `DD-UC-NNN` de `docs/design/` como código real (esqueleto de proyecto, features, UI Angular) vía prompts `PR-IMPL-NNN` | Código en `backend/`, `frontend/`, `infra/`; contratos en `docs/prompts/impl/` (área `IMPL`) y `prompts/` (resto de áreas); DTOs en `docs/dtos_EduSync.md` |
...
# Task
Implementa FSD-UC-013 segun docs/design/DD-UC-015.md §2: backend
(PeriodoEvaluacion Aggregate independiente, seed 3 trimestres al crear
GestionEscolar, GET /gestiones-escolares/{id}, POST/GET .../periodos,
PATCH/DELETE y PATCH .../estado secuencial, V9 + RLS) Y frontend
(/academico/gestiones-escolares/:id/periodos) en el mismo prompt.

# Context
- Fuente: docs/design/DD-UC-015.md (orden, freeze, seed; ver §2/§3).
- FSD: docs/product/FSD.md §4.6.3. ADRs 0001/0008/0009/0011/0012/0013.
- Precedentes: GestionEscolarController, curso-paralelos.page.ts.
- Restricciones: tenantId del contexto; academico no importa identidad;
  no FSD-UC-014/015/016; no seed de secciones; no audit_log; no PII.

# Reasoning
1. Domain PeriodoEvaluacion + estados + excepciones.
2. V9 RLS; seed en CrearGestionEscolarService.
3. GET gestion/{id}; CRUD periodos; PATCH estado secuencial.
4. UI detalle anidado; enlace desde lista de gestiones.
5. mvn test + ng build + ModularityTests verde.

# Stop condition
Detente cuando el seed crea 3 periodos, E_PERIODO_NO_SECUENCIAL y
E_PERIODOS_SOLAPADOS/INMUTABLES pasan, GET /{id} existe, ModularityTests
sin ciclo y ng build verde. No implementes secciones, calculo ni
audit_log. No edites docs/baseline/**.

# Output
backend academico (delta) + V9; frontend gestion-periodos.page.ts;
delta lista de gestiones y app.routes.

# Invariants
- tenantId nunca del cliente.
- Apertura secuencial BR-017.
- Freeze si hay un ABIERTO.
- N >= 1. Cross-tenant → 404.

# Failure modes
- E_TENANT_DESDE_CLIENTE: rechazar.
- E_CICLO_MODULO: academico no importa identidad/plataforma.
- E_SEED_OMITIDO: rechazar.
- E_SECCIONES_COLADAS: revertir (FSD-UC-014).
- E_CALCULO_COLADO: revertir (FSD-UC-016).
- E_SECUENCIA_OMITIDA: rechazar.
- E_PERIODOS_EMBEBIDOS: revertir.
- E_AUDIT_LOG_INVENTADO: revertir, ADR-0009 §3 punto 5.
- E_PII_EN_LOG: rechazar.
- E_ROLE_GUARD_REESCRITO: no tocar role.guard.ts.
- E_BASELINE_TOCCADO: revertir.
```
...
# Task
Implementa FSD-UC-014 segun docs/design/DD-UC-016.md §2: backend
(SeccionEvaluacion Aggregate independiente a nivel de gestion, seed 4
secciones al crear GestionEscolar, GET/PUT/POST .../secciones, PATCH
/secciones-evaluacion/{id}, V10 + RLS) Y frontend
(/academico/gestiones-escolares/:id/secciones) en el mismo prompt.

# Context
- Fuente: docs/design/DD-UC-016.md (PUT atomico, freeze sticky, seed; ver §2/§3).
- FSD: docs/product/FSD.md §4.6.4. ADRs 0001/0008/0009/0011/0012/0013.
- Precedentes: CrearGestionEscolarService (seed periodos), gestion-periodos.page.ts.
- Restricciones: tenantId del contexto; academico no importa identidad;
  no FSD-UC-015/016; no Evaluacion ni calculo; no audit_log; no PII.

# Reasoning
1. Domain SeccionEvaluacion + excepciones de peso/suma/freeze.
2. V10 RLS; seed 4 en CrearGestionEscolarService.
3. GET/PUT/POST secciones; PATCH item; freeze sticky; delta abrir periodo.
4. UI tabla + PUT; enlace desde lista de gestiones.
5. mvn test + ng build + ModularityTests verde.

# Stop condition
Detente cuando el seed crea 4 secciones Σ=100, E_SUMA_SECCIONES_INVALIDA,
E_PESO_INVALIDO y E_SECCIONES_INMUTABLES (ABIERTO y CERRADO) pasan,
ModularityTests sin ciclo y ng build verde. No implementes Evaluacion,
calculo ni audit_log. No edites docs/baseline/**.

# Output
backend academico (delta) + V10; frontend gestion-secciones.page.ts;
delta lista de gestiones y app.routes.

# Invariants
- tenantId nunca del cliente.
- Plantilla por gestion (no por periodo).
- Σ nota = 100. Freeze sticky.
- Cross-tenant → 404.
...
- E_CICLO_MODULO: academico no importa identidad/plataforma.
- E_SEED_OMITIDO: rechazar.
- E_EVALUACION_COLADA: revertir (FSD-UC-015/016).
- E_PESO_SEPARADO: revertir.
- E_SECCIONES_POR_PERIODO: revertir.
- E_SECCIONES_EMBEBIDAS: revertir.
- E_FREEZE_NO_STICKY: rechazar.
- E_AUDIT_LOG_INVENTADO: revertir, ADR-0009 §3 punto 5.
- E_PII_EN_LOG: rechazar.
- E_ROLE_GUARD_REESCRITO: no tocar role.guard.ts.
- E_BASELINE_TOCCADO: revertir.
...
# Task
Implementa FSD-UC-015 segun docs/design/DD-UC-017.md §2: backend
(Evaluacion Aggregate independiente, puntajeMaximo = seccion.nota,
POST /evaluaciones, GET /materias/{id}/evaluaciones, GET /materias/mias,
V11 + RLS) Y frontend (/academico/mis-materias +
/academico/materias/:id/evaluaciones) en el mismo prompt.

# Context
- Fuente: docs/design/DD-UC-017.md (puntajeMaximo derivado, primera UI
  PROFESOR; ver §2/§3).
- FSD: docs/product/FSD.md §4.6.5. ADRs 0001/0008/0009/0010/0011/0012/0013.
- Precedentes: AsignacionMateriaProfesor, PeriodoEvaluacion, SeccionEvaluacion.
- Restricciones: tenantId y actorId del contexto/JWT; academico no importa
  identidad; no FSD-UC-016; no Calificacion ni round/floor; no audit_log; no PII.

# Reasoning
1. Domain Evaluacion + excepciones A1/periodo/seccion-gestion.
2. V11 RLS; POST deriva puntajeMaximo; GET /materias/mias.
3. PROFESOR solo materias asignadas (404 si no); ADMIN override.
4. UI Mis materias + evaluaciones; login PROFESOR; no tocar role.guard.ts.
5. mvn test + ng build + ModularityTests verde.

# Stop condition
Detente cuando POST crea evals con puntajeMaximo = seccion.nota,
E_MATERIA_SIN_PROFESOR y E_PERIODO_NO_ABIERTO pasan, ModularityTests
sin ciclo y ng build verde. No implementes Calificacion, calculo ni
audit_log. No edites docs/baseline/**.

# Output
backend academico (delta) + V11; frontend mis-materias.page.ts y
materia-evaluaciones.page.ts; deltas shell, login, materia-detalle, routes.

# Invariants
- tenantId nunca del cliente. actorId nunca del body.
- puntajeMaximo solo de seccion.nota.
- Cross-tenant y Profesor no asignado → 404.

# Failure modes
- E_TENANT_DESDE_CLIENTE: rechazar.
- E_ACTOR_DESDE_CLIENTE: rechazar.
- E_CICLO_MODULO: academico no importa identidad/plataforma.
- E_PUNTAJE_DEL_CLIENTE: revertir.
- E_CALIFICACION_COLADA: revertir (FSD-UC-016).
- E_TIPO_EVALUACION: revertir.
- E_AGREGADO_EMBEBIDO: revertir.
- E_AUDIT_LOG_INVENTADO: revertir, ADR-0009 §3 punto 5.
- E_PII_EN_LOG: rechazar.
- E_ROLE_GUARD_REESCRITO: no tocar role.guard.ts.
- E_BASELINE_TOCCADO: revertir.
...
### PR-IMPL-018 — Académico: Calificaciones + CalculoNotas (backend + UI)
...
# Task
Implementa FSD-UC-016 segun docs/design/DD-UC-018.md §2: backend
(CalificacionEvaluacion Aggregate, motor CalculoNotas round HALF_UP sin
floor, PUT/GET /evaluaciones/{id}/calificaciones, GET nota-provisional,
V12 + RLS) Y frontend (matriz
/academico/materias/:id/evaluaciones/:evaluacionId/calificaciones)
en el mismo prompt.

# Context
- Fuente: docs/design/DD-UC-018.md (notas + motor + matriz; ver §2/§3).
- FSD: docs/product/FSD.md §4.6.6. ADRs 0001/0008/0009/0010/0011/0012/0013.
- Precedentes: Evaluacion, Inscripcion, AsignacionMateriaCurso.
- Restricciones: tenantId/actorId del contexto/JWT; academico no importa
  identidad; no floor/notassie; no TipoEvaluacion; no audit_log; no PII.

# Reasoning
1. Domain CalificacionEvaluacion + CalculoNotas (canónico 37.50→93→31).
2. V12 RLS; PUT upsert lote; GET nómina; GET nota-provisional.
3. E_RANGO_INVALIDO; E_ESTUDIANTE_NO_INSCRITO; periodo ABIERTO / eval ACTIVA.
4. UI matriz; enlace desde MateriaEvaluacionesPage; no tocar role.guard.ts.
5. mvn test + ng build + ModularityTests verde.

# Stop condition
Detente cuando el canónico ADR-0013 pasa, E_RANGO_INVALIDO rechaza 46,
ModularityTests sin ciclo y ng build verde. No implementes floor/SIE,
audit_log ni promedios materializados. No edites docs/baseline/**.

# Output
backend academico (delta) + V12; frontend evaluacion-calificaciones.page.ts;
deltas materia-evaluaciones, routes.

# Invariants
- tenantId nunca del cliente. actorId nunca del body.
- Cálculo solo en dominio CalculoNotas. Sin floor().
- Cross-tenant y Profesor no asignado → 404.

# Failure modes
- E_TENANT_DESDE_CLIENTE / E_ACTOR_DESDE_CLIENTE: rechazar.
- E_CICLO_MODULO: academico no importa identidad/plataforma.
- E_FLOOR_EN_GENERICO / E_CALCULO_FUERA_DOMINIO: revertir.
- E_NOMBRE_CALIFICACION_SIE: renombrar a CalificacionEvaluacion.
- E_PROMEDIOS_MATERIALIZADOS / E_NOTASSIE_COLADO: revertir.
- E_AUDIT_LOG_INVENTADO: revertir, ADR-0009 §3 punto 5.
- E_PII_EN_LOG: rechazar.
- E_ROLE_GUARD_REESCRITO: no tocar role.guard.ts.
- E_BASELINE_TOCCADO: revertir.
...
### PR-IMPL-019 — Académico: relajación de reglas de flujo + visibilidad de Gestión Escolar por rol
...
elimina la maquina de estados/freeze/secuencialidad de GestionEscolar,
PeriodoEvaluacion y SeccionEvaluacion (conservando suma-100, no-solape y
minimo-1-periodo); añade visibilidad ACTIVA-only de GestionEscolar para
SECRETARIA/PROFESOR/ASESOR; nuevo PATCH nombre/fechas de GestionEscolar.
Backend Y frontend en el mismo prompt.

# Context
- Fuente: docs/design/DD-UC-019.md (decisiones usuario: ACTIVA, admin
  bypasea reglas, nuevo PATCH; ver §2/§3).
- FSD: docs/product/FSD.md §4.6.2/§4.6.3/§4.6.4. ADRs 0001/0008..0013;
  crea ADR-0014 (relaja ADR-0013 §3.1.4/3.1.5/§3.2.2).
- Precedentes: GestionEscolar/PeriodoEvaluacion/SeccionEvaluacion
  (DD-UC-008/015/016); patron 404-no-403 de aislamiento cross-tenant.
- Restricciones: tenantId del contexto; conservar exigirSumaCien/
  exigirSinSolape/PeriodoUnicoException; no tocar CalculoNotas; no
  floor/notassie/audit_log; no PII.

# Reasoning
1. Domain: GestionEscolar/PeriodoEvaluacion sin maquina de estados; borrar
   5 excepciones de transicion/freeze.
...
3. Nuevo ActualizarGestionEscolarUseCase/Command/Service (PATCH nombre/
   fechas); nuevo GestionEscolarVisibilidad + actorVeTodas en 4 interfaces
   de lectura.
4. Controller: Authentication + ASESOR + nuevo PATCH; limpiar exception
   handlers de codigos muertos en los 3 controladores.
5. Frontend: rutas ampliadas a 4 roles; esAdmin en las 3 paginas; dialogos
   Editar; quitar gating obsoleto (puedeAbrir/todosPendiente/congelada).
6. mvn test + ng build + ModularityTests verde.

# Stop condition
Detente cuando abrir T2 con T1 ABIERTO ya no rechaza, PUT/PATCH de
secciones/periodos con ABIERTO/CERRADO ya no rechaza, suma!=100 y solape
siguen en 422, SECRETARIA/PROFESOR/ASESOR solo ven ACTIVA (404 en detalle
si no lo es), mvn test y ng build verdes. No toques CalculoNotas ni
CalificacionEvaluacion. No edites docs/baseline/**.

# Output
backend academico (delta, 5 excepciones eliminadas, 2 nuevos archivos
application, 1 nuevo DTO) + tests actualizados; frontend 3 paginas +
routes + shell (delta); docs/adr/0014-*.md.

# Invariants
- tenantId nunca del cliente.
- Se conservan exigirSumaCien, exigirSinSolape, PeriodoUnicoException.
- Actor no-admin sobre gestion no ACTIVA → 404, nunca 403.
- No se toca CalculoNotas/CalificacionEvaluacion.

# Failure modes
- E_TENANT_DESDE_CLIENTE: rechazar.
- E_SUMA_CIEN_ELIMINADA: revertir — invariante de integridad, no de flujo.
- E_403_EN_VEZ_DE_404: corregir, viola el patron 404-no-403.
- E_CALCULO_TOCADO: revertir, fuera de alcance.
- E_ROLE_GUARD_REESCRITO: no tocar role.guard.ts.
...
(Alternativa C): (1) verifica un BOM Spring AI compatible con Boot 4.1.0
/ Jakarta EE 11 / Java 25 — si no existe, DETENTE (E_BOM_INCOMPATIBLE);
(2) anade BOM + starter(s) en backend/pom.xml; (3) reescribe
OllamaLlmAdapter y OpenWebUiLlmAdapter via Spring AI; (4) conserva
edusync.ai.* y las env actuales; (5) preserva HTTP/1.1 si Open WebUI
lo exige. NO toques LlmPort, ChatConLlmService, domain/, el contrato
POST /api/v1/ai/chat, frontend/ ni docs/baseline/**.

# Context
- Documento fuente: docs/design/DD-UC-022.md §1/§2.
- ADR: ADR-0017 (Spring AI solo en infrastructure, detras de LlmPort).
- Spike actual: com.edusync.shared.ai (RestClient ad-hoc).
- Default: provider=ollama, model=llama3.1:latest.
- Restricciones: AGENTS.md §4/§5/§7, NFR-007, ModularityTests 7/7.

# Reasoning
1. Verificar BOM vs Boot 4.1.0; si no, stop.
2. Anadir BOM + starters minimos (sin vector store / MCP).
3. Beans ChatClient desde AiProperties; no exigir rename de env.
4. Adaptadores mapean completar() -> RespuestaLlm; vacio/error ->
   LlmNoDisponibleException; logs solo modelo + chars.
5. Tests de adaptador con ChatClient mock; ChatConLlmServiceTest intacto.
6. mvn test incluyendo ModularityTests.

# Stop condition
Detente cuando el BOM este justificado (o se haya escalado), no haya
imports org.springframework.ai en domain/application, el contrato de
chat no haya cambiado, edusync.ai.* siga siendo la interfaz del
operador, no se loguee PII/prompt/key, y mvn test este verde.
NO agregues endpoints, NO toques frontend/, NO edites docs/baseline/**.

# Output
backend/pom.xml (delta), AiConfig.java (delta), OllamaLlmAdapter.java
(delta), OpenWebUiLlmAdapter.java (delta), tests de adaptador.

# Invariants
- LlmPort.completar(String) no cambia.
- Spring AI no entra a domain/ ni application/.
- POST /api/v1/ai/chat conserva firma y status codes.
- Sin PII/RUDE/notas/prompt/API key en logs INFO+.
- docs/baseline/** no se edita.

# Failure modes
- E_BOM_INCOMPATIBLE: detener y escalar; no pinnear Boot 3.
- E_SPRING_AI_EN_DOMAIN: revertir.
- E_CONTRATO_CHAT_CAMBIADO: revertir.
- E_PII_EN_LOG: corregir antes de merge.
- E_BASELINE_TOCADO: revertir.
```
...
arquitectura hexagonal (Ports & Adapters) y adopcion de librerias de
...
# Context
- Fuente: spike com.edusync.shared.ai; DTI §9 historico "sin IA en
  runtime"; AGENTS.md §4/§5/§7.
- Stack vivo: Java 25, Spring Boot 4.1.0 (ADR-0008).
- La version del BOM se fija en PR-IMPL-022, no en el ADR.
- No tocar docs/baseline/**.

# Reasoning
1. Contexto: cliente JSON propio vs. DTI §9; hexagono ya tiene LlmPort.
2. Alternativas: (A) statu quo RestClient, (B) ChatClient en
   application/, (C) Spring AI solo en adaptadores, (D) proveedor cloud.
3. Decidir C: el dolor es el I/O HTTP, no el puerto.
4. Consecuencias, impacto, reversion (BOM incompatible -> volver a A).
5. Enlazar DD-UC-022 / PR-IMPL-022.

# Stop condition
Detente cuando el ADR tenga las 9 secciones, estado Aceptada,
Alternativa C explicita, y quede claro que domain/application no
importan Spring AI ni se edita docs/baseline/**.
...
# Invariants
- Spring AI no entra a domain/ ni application/.
- POST /api/v1/ai/chat y edusync.ai.* no se redefinen como "a cambiar".
- Al menos 3 alternativas.
- Sin editar docs/baseline/**.

# Failure modes
- E_ALTERNATIVA_INSUFICIENTE: ampliar.
- E_SPRING_AI_EN_DOMAIN: revertir.
- E_BOM_PINNEADO_A_CIEGAS: el pin queda para PR-IMPL-022.
- E_BASELINE_TOCADO: revertir.
```
...
| Design Doc `DD-UC-015` + `ADR-0001` + `ADR-0008` + `ADR-0009` + `ADR-0011` + `ADR-0013` + `docs/product/FSD.md` (`FSD-UC-013`) | `DD-UC-015, ADR-0001, ADR-0008, ADR-0009, ADR-0011, ADR-0013, FSD-UC-013` | PR-IMPL-015 | `dev-agent` | `PeriodoEvaluacion` (backend + consola Angular fullstack) — **ejecutado** 21/08/2026 | `backend/.../PeriodoEvaluacion*`, `V9__academico_periodo_evaluacion.sql`, `frontend/.../gestion-periodos.page.ts`; prompt en `docs/prompts/impl/PR-IMPL-015.md` |
| Design Doc `DD-UC-016` + `ADR-0001` + `ADR-0008` + `ADR-0009` + `ADR-0011` + `ADR-0013` + `docs/product/FSD.md` (`FSD-UC-014`) | `DD-UC-016, ADR-0001, ADR-0008, ADR-0009, ADR-0011, ADR-0013, FSD-UC-014` | PR-IMPL-016 | `dev-agent` | `SeccionEvaluacion` (backend + consola Angular fullstack) — **ejecutado** 21/08/2026 | `backend/.../SeccionEvaluacion*`, `V10__academico_seccion_evaluacion.sql`, `frontend/.../gestion-secciones.page.ts`; prompt en `docs/prompts/impl/PR-IMPL-016.md` |
...
| Design Doc `DD-UC-022` + `ADR-0017` + `docs/product/FSD.md` (`NFR-007`) | `DD-UC-022, ADR-0017, NFR-007` | PR-IMPL-022 | `dev-agent` | Spring AI 2.0.0 como cliente HTTP/LLM detrás de `LlmPort` — **ejecutado** 13/09/2026 | `docs/design/DD-UC-022.md`, `docs/prompts/impl/PR-IMPL-022.md`, `docs/adr/0017-adopcion-spring-ai-cliente-llm.md`, `backend/pom.xml` (`spring-ai-bom` 2.0.0), `shared.ai.infrastructure/**` |
| Design Doc `DD-UC-022` §1/§2 + `AGENTS.md` §4/§5/§7 | `DD-UC-022, NFR-007` | PR-ADR-010 | `arch-agent` | `ADR-0017` (Spring AI detrás de `LlmPort`) — **Aprobado** | `docs/adr/0017-adopcion-spring-ai-cliente-llm.md`, `prompts/PR-ADR-010.md` |
...
| v2.30 | 21/08/2026 | Rodrigo Aspeti | Incorporación de `PR-ADR-006` (`docs/adr/0013-modelo-generico-periodos-secciones-calculo.md`): resuelve `ADR-0009` §3 puntos 1–4 (modelo genérico único, apertura secuencial, `round` HALF_UP, Σ secciones = 100). Cabecera `v2.29 → v2.30`. Índice ampliado. Matriz `arch-agent` incluye `PR-ADR-006`. Propagado a `docs/product/{BRD,PRD,FSD,DTP}.md` y `AGENTS.md` v0.41. Total prompt-contratos activos: 51 → 52. |
| v2.31 | 21/08/2026 | Rodrigo Aspeti | Decimoquinta materialización del área `IMPL` (**diseño aprobado, ejecución pendiente**): `PR-IMPL-015` (módulo `academico`: `PeriodoEvaluacion` — backend hexagonal y consola Angular en el mismo prompt), derivado de `docs/design/DD-UC-015.md` (`FSD-UC-013`) y de `ADR-0013`. Decisiones explícitas: Aggregate independiente con `orden`; seed 3 trimestres al crear gestión; freeze si hay un `ABIERTO`; `GET /gestiones-escolares/{id}`; RBAC escrituras `ADMIN`. Índice ampliado. Flowchart nodo `IMPL015`. Matriz `dev-agent` ampliada. Contrato inline agregado. Trazabilidad ampliada. Archivo `docs/prompts/impl/PR-IMPL-015.md` materializado. Total prompt-contratos activos: 52 → 53. |
| v2.32 | 21/08/2026 | Rodrigo Aspeti | **Ejecución de `PR-IMPL-015`**: pasa de "Aprobado (prompt)" a **"Ejecutado"** — código real de Periodos (`PeriodoEvaluacion`, seed, freeze, secuencia, `V9`, consola Angular). `mvn test` **200/200** (incluye `ModularityTests` 7/7); `ng build` verde. `FSD-UC-013` cierra implementación **completa** (backend + UI). Sin filas nuevas en el índice. Total prompt-contratos activos: 53 (sin cambio). |
| v2.33 | 21/08/2026 | Rodrigo Aspeti | Decimosexta materialización del área `IMPL` (**diseño aprobado, ejecución pendiente**): `PR-IMPL-016` (módulo `academico`: `SeccionEvaluacion` — backend hexagonal y consola Angular en el mismo prompt), derivado de `docs/design/DD-UC-016.md` (`FSD-UC-014`) y de `ADR-0013`. Decisiones explícitas: Aggregate independiente a nivel de gestión; seed 4 secciones al crear gestión; PUT atómico (Σ `nota` = 100); freeze sticky (ABIERTO o CERRADO); RBAC escrituras `ADMIN`. Índice ampliado. Flowchart nodo `IMPL016`. Matriz `dev-agent` ampliada. Contrato inline agregado. Trazabilidad ampliada. Archivo `docs/prompts/impl/PR-IMPL-016.md` materializado. Total prompt-contratos activos: 53 → 54. |
| v2.34 | 21/08/2026 | Rodrigo Aspeti | **Ejecución de `PR-IMPL-016`**: pasa de "Aprobado (prompt)" a **"Ejecutado"** — código real de Secciones (`SeccionEvaluacion`, seed 4, PUT atómico, freeze sticky, `V10`, consola Angular). `mvn test` **215/215** (incluye `ModularityTests` 7/7); `ng build` verde. `FSD-UC-014` cierra implementación **completa** (backend + UI). Sin filas nuevas en el índice. Total prompt-contratos activos: 54 (sin cambio). |
| v2.35 | 21/08/2026 | Rodrigo Aspeti | Decimoséptima materialización del área `IMPL` (**diseño aprobado, ejecución pendiente**): `PR-IMPL-017` (módulo `academico`: `Evaluacion` — backend hexagonal y consola Angular en el mismo prompt), derivado de `docs/design/DD-UC-017.md` (`FSD-UC-015`) y de `ADR-0013`. Decisiones explícitas: Aggregate independiente; `puntajeMaximo` derivado de `seccion.nota`; A1 `409 E_MATERIA_SIN_PROFESOR`; primera consola `PROFESOR` (Mis materias); calificaciones de estudiante y motor `round` diferidos a `FSD-UC-016`. Índice ampliado. Flowchart nodo `IMPL017`. Matriz `dev-agent` ampliada. Contrato inline agregado. Trazabilidad ampliada. Archivo `docs/prompts/impl/PR-IMPL-017.md` materializado. Total prompt-contratos activos: 54 → 55. |
| v2.36 | 21/08/2026 | Rodrigo Aspeti | **Ejecución de `PR-IMPL-017`**: fila pasa a Ejecutado. `mvn test` 228/228; `ng build` verde. `FSD-UC-015` completo backend+UI. Total prompt-contratos activos: 55 (sin fila nueva). |
...
| v2.43 | 13/09/2026 | Rodrigo Aspeti | Vigésima segunda materialización del área `IMPL` (**diseño aprobado, prompt aprobado, ejecución pendiente**): `PR-IMPL-022` (backend: adoptar Spring AI como cliente HTTP/LLM **detrás** de `LlmPort`, sin cambiar `POST /api/v1/ai/chat` ni `edusync.ai.*`), derivado de `docs/design/DD-UC-022.md` (`NFR-007`, refactor del spike `shared.ai`) y de `ADR-0017` (nuevo: Alternativa C; BOM a fijar solo tras verificar Boot 4.1.0). `PR-IMPL-021` / `PR-ADR-009` no se materializan aquí (reservados por `DD-UC-021` / `ADR-0016`). Índice ampliado con `PR-IMPL-022` (Aprobado (prompt)) y `PR-ADR-010` (Aprobado). Flowchart nodo `IMPL022`. Matriz `dev-agent`/`arch-agent` ampliadas. 2 contratos inline agregados. Trazabilidad ampliada. Archivos `docs/prompts/impl/PR-IMPL-022.md` y `prompts/PR-ADR-010.md` materializados. Total prompt-contratos activos: 60 → 62. |
| v2.44 | 13/09/2026 | Rodrigo Aspeti | **Ejecución de `PR-IMPL-022`**: BOM `spring-ai-bom` 2.0.0 (Boot 4.1.0 / Jakarta EE 11 / Java 25). Adaptadores `ChatClient`; `spring.ai.model.chat=none`. `mvn test` 250/250 (incluye `ModularityTests` 7/7). Estado fila `PR-IMPL-022` → Ejecutado. Trazabilidad actualizada. Sin filas nuevas (62 contratos). |
