...
- [x] Prompt `PR-IMPL-003` creado en `docs/prompts/impl/` y registrado en `PROMPT_MAPPING.md`.
- [x] `PR-IMPL-003` **ejecutado** (19/07/2026): módulo `plataforma` real (dominio `Tenant`/`EstadoTenant`, casos de uso `Registrar`/`CambiarEstado`/`CrearAdmin`Tenant, `VencimientoSchedulerService`+Job, `TenantController` con RBAC `SYSADMIN`, persistencia JPA, `V3__plataforma_tenant.sql`); `identidad.TenantConsultaPort` (puerto público, resuelve el ciclo Spring Modulith documentado en su Javadoc — implementación real vive en `plataforma.infrastructure`, no en `plataforma` como sugería §2, ver nota de diseño en el propio puerto) y enforcement de `BR-014` en `AutenticarUsuarioService`/`AuthController` (`E_TENANT_NO_ACTIVO`, HTTP 403).
- [x] Tests/evals definidos (§6) y pasando: 45/45 tests verdes (`TenantTest` 5, `RegistrarTenantServiceTest` 1, `CambiarEstadoTenantServiceTest` 2, `CrearAdminTenantServiceTest` 2, `VencimientoSchedulerServiceTest` 2 con `Clock` simulado, `TenantIntegrationTest` 3 con Testcontainers — cubre alta tenant+admin en dos llamadas, bloqueo de login por `BR-014`, RBAC `SYSADMIN`, `E_SUSCRIPCION_INCOMPLETA` 422 — más los 34 tests preexistentes de `identidad`/`ModularityTests`).
- [x] `ModularityTests` en verde (7/7) con la comunicación bidireccional `plataforma`↔`identidad` vía `UsuarioCreacionPort`/`TenantConsultaPort`, sin ciclos de módulo.
- [x] DTP actualizado vía `dtp-sync` (`docs/product/DTP.md` v1.9).
...
| v1.1 | 19/07/2026 | Rodrigo Aspeti | **Ejecución de `PR-IMPL-003`**: módulo `plataforma` implementado con código real (dominio `Tenant` con mutabilidad controlada de `estado`, casos de uso de alta/cambio de estado/alta de admin/scheduler de vencimiento, `TenantController` con `@PreAuthorize("hasRole('SYSADMIN')")`, persistencia JPA, migración `V3__plataforma_tenant.sql`) y enforcement de `BR-014` en el módulo `identidad`. **Refinamiento de diseño respecto a §2**: `TenantConsultaPort` se declaró en la raíz de `identidad` (no de `plataforma`) para evitar un ciclo de módulos rechazado por `ApplicationModules.verify()` de Spring Modulith (`plataforma` ya depende de `identidad` vía `UsuarioCreacionPort`); la implementación real vive en `plataforma.infrastructure.adapter.out.port.TenantConsultaPortImpl` — funcionalmente idéntico al diseño original (Open Host Service, consulta sincrónica), documentado en el Javadoc del puerto. Corrección técnica encontrada durante la ejecución: `HttpStatus.UNPROCESSABLE_ENTITY` fue renombrado a `HttpStatus.UNPROCESSABLE_CONTENT` en Spring Framework 7.x (usado por Spring Boot 4.1) — `TenantController` y `TenantIntegrationTest` actualizados. Verificación: `mvn test` → 45/45 tests verdes (incluye `ModularityTests` 7/7). §7 DoD marcado completo. Sincronizado con `docs/PROMPT_MAPPING.md` v2.6, `docs/product/DTP.md` v1.9 y `AGENTS.md` v0.22. |
