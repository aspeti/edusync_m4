...
> BR-013..BR-024 son aditivos: no modifican ni contradicen BR-001..BR-012. Cubren la capa de plataforma SaaS y los módulos configurables descritos en el nuevo diseño funcional. `ADR-0013` resuelve los puntos 1–4 de `ADR-0009` §3 (modelo único, secuencialidad, redondeo genérico, suma 100). El punto 5 (gobernanza) sigue pendiente.
...
| BR-016 | Cada Unidad Educativa debe poder crear una Gestión Escolar con nombre, fecha de inicio, fecha de finalización y estado (Planificación / Activa / Cerrada). Al crearla, el sistema siembra 3 periodos y 4 secciones de evaluación (defaults editables). | Must | Es la unidad organizativa raíz de la que dependen periodos, secciones, cursos e inscripciones de un ciclo escolar (`ADR-0013`). | 100 % de las Gestiones Escolares creadas tienen los 4 campos obligatorios y el seed de 3 periodos + 4 secciones (suma de `nota` = 100). |
| BR-017 | Cada Gestión Escolar tiene un número **configurable** de periodos de evaluación (N ≥ 1; seed = 3). Cada periodo tiene nombre, fecha de inicio, fecha de fin y estado (`PENDIENTE`/`ABIERTO`/`CERRADO`). La apertura es **secuencial**: el periodo *k* no abre si el *k−1* no está cerrado (`ADR-0013`). | Must | Generaliza `RB-05`/`BR-006` a N periodos sin hardcodear tres trimestres. | 0 aperturas del periodo *k* con el *k−1* no cerrado. El sistema opera con N ≠ 3 sin cambio de código. |
| BR-018 | Cada Gestión Escolar tiene una **plantilla única** de secciones de evaluación (no por periodo), cada una con nombre, orden y `nota` (puntos sobre 100). La suma de `nota` **es exactamente 100**. Defaults: Ser 5, Saber 45, Hacer 40, Autoevaluación 10. La plantilla es inmutable desde que el primer periodo está `ABIERTO` (`ADR-0013`). | Must | Un solo esquema pedagógico para todos los trimestres/bimestres; evita pesos distintos por periodo. | 100 % de las gestiones tienen Σ `nota` = 100. 0 ediciones de secciones con algún periodo `ABIERTO`. |
| BR-019 | Las evaluaciones de una materia usan las secciones de la gestión. Cada sección admite 1..N evaluaciones. Toda evaluación se califica en `[0, seccion.nota]` (si Saber = 45, todas sus evals son 0–45). | Must | El docente carga varias evaluaciones por sección en la misma escala; no se parten los 45 puntos entre evals. | 100 % de las calificaciones persistidas están en `[0, seccion.nota]`. 0 evaluaciones con `puntajeMaximo` distinto al `nota` de su sección. |
| BR-020 | El sistema recalcula automáticamente, en el motor de dominio: `nota_seccion = (Σ notas) / n` (2 decimales HALF_UP); `nota_periodo = round(Σ nota_seccion)` a entero HALF_UP; `promedio_gestion = round((Σ nota_periodo_o_cero) / N)` a entero, **visible** aunque falten periodos (`PROVISIONAL`). No se usa `floor()` en el modelo genérico (`ADR-0013`). | Must | Consolidación automática con la fórmula confirmada por el negocio (ejemplo: Saber 35 y 40 → 37.5; periodo 92.5 → 93; gestión 93/3 → 31). | El ejemplo canónico y el 100 % de los casos de prueba de dominio coinciden con esa fórmula. |
...
> **Pendiente de definición (`ADR-0009` §3 punto 5 / `ADR-0013` §3.5):** la gobernanza (auditoría inalterable, inmutabilidad post-cierre, ventana de corrección retroactiva) de BR-013..BR-024 no está definida todavía; no debe asumirse equivalente a BR-005/BR-009/BR-011 (Perfil Bolivia SIE) sin una decisión explícita de negocio. Los puntos 1–4 de `ADR-0009` §3 están resueltos en `ADR-0013`.
...
> **RB-01..RB-11 son del Perfil Bolivia SIE** y siguen vigentes sin cambios. Sus equivalentes genéricos están en `ADR-0013` / BR-017..BR-020: apertura secuencial a N periodos (RB-05); promedio de gestión **visible** con `/ N` y periodos vacíos = 0, marcado `PROVISIONAL` (**distinto** de RB-11, que oculta el anual hasta 3 trimestres cerrados); `round` HALF_UP en el genérico (**no** `floor()`, que permanece en RB-08 / `FSD-UC-003`); suma de `nota` de secciones = 100 (BR-018).
...
| BR-016..BR-017 | `ADR-0009`, `ADR-0013` | FSD-UC-012, FSD-UC-013 | PRD-REQ-022, PRD-REQ-023 | FSD-UC-012, FSD-UC-013 |
| BR-018..BR-019 | `ADR-0009`, `ADR-0013` | FSD-UC-014, FSD-UC-015 | PRD-REQ-024, PRD-REQ-025 | FSD-UC-014, FSD-UC-015 |
| BR-020 | `ADR-0009`, `ADR-0013` | FSD-UC-016 | PRD-REQ-026 | FSD-UC-016 |
...
| v3.2 | 21/08/2026 | Rodrigo Aspeti | Resolución de `ADR-0009` §3 puntos 1–4 vía `ADR-0013`: BR-016..BR-020 reescritos (seed 3 periodos + 4 secciones, plantilla de secciones por gestión con suma 100, escala `[0, nota]`, promedio simple, `round` entero HALF_UP, promedio de gestión `/ N` visible `PROVISIONAL`, apertura secuencial). RB-01..RB-11 Bolivia intactos. Punto 5 (gobernanza) sigue pendiente. |
