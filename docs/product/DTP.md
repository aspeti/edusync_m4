...
version: v1.8                  # versiona la implementación, no el diseño de M4
fecha: "28/05/2026"
...
| `FSD-UC-011` (Gestión de Tenants y Suscripciones) | `DD-UC-003` | en progreso | `release/3.0.0` | `ModularityTests`: 7/7 verde (solo bootstrap, sin lógica de `Tenant` todavía) | `DD-UC-003` (módulo `plataforma`: `Tenant`, ciclo de suscripción, scheduler de vencimiento, `TenantConsultaPort`) creado y aprobado; `PR-IMPL-003` generado, **ejecución de código de dominio pendiente** (solo existe el paquete vacío `plataforma/` tras `PR-IMPL-001`). Consume `UsuarioCreacionPort` de `identidad` (`DD-UC-002`) para crear el primer `ADMIN` del tenant en una llamada REST separada. Diseño del tenant "demo" diferido a un Design Doc de seguimiento aún sin crear |
...
| `FSD-UC-021` (Usuarios y Roles / login) | `DD-UC-002` | en progreso | `release/3.0.0` | 27/27 tests verde (`AutenticarUsuarioServiceTest` 4, `CrearUsuarioServiceTest` 3, `AuthIntegrationTest` 4 con Testcontainers PostgreSQL 15, `UsuarioTest` 6, `JwtTokenProviderTest` 3, `ModularityTests` 7); smoke test manual de Swagger UI/OpenAPI en verde | **`PR-IMPL-002` ejecutado** (18–19/07/2026): módulo `identidad` real y funcional (dominio `Usuario`/`UsuarioRol`, login JWT, `TenantContextProvider`, seed `SYSADMIN`, `UsuarioCreacionPort`), con `ADR-0012` (Lombok/springdoc-openapi/Bean Validation) aplicado sobre el mismo código. Cubre solo la autenticación (parcial de `FSD-UC-021`); el CRUD administrativo completo (alta desde Admin, `PATCH roles`, activar/desactivar, restablecer contraseña) es `DD-UC-004`, todavía sin crear |
...
Estado actual: la cadena existe completa hasta `Design Doc` para `FSD-UC-011`/`FSD-UC-021` → `docs/product/FSD.md` → `docs/design/{DD-UC-001,DD-UC-002,DD-UC-003}.md` → `docs/prompts/impl/{PR-IMPL-001,PR-IMPL-002,PR-IMPL-003}.md` (registrados en `docs/PROMPT_MAPPING.md` v2.4) → `ADR-0011`/`ADR-0010`/`ADR-0009`/`ADR-0001`. **`PR-IMPL-001` y `PR-IMPL-002` ya se ejecutaron**: `backend/`, `frontend/` e `infra/docker-compose.yml` existen con el esqueleto de bootstrap (18/07/2026), y el módulo `identidad` tiene lógica de dominio real y funcional — login JWT, `Usuario`/`UsuarioRol`, `TenantContextProvider` real (18–19/07/2026), con `ADR-0012` (Lombok/springdoc-openapi/Bean Validation) aplicado sobre el mismo código; `Tests/Evals` = 27/27 tests verdes (incluye `ModularityTests` 7/7), `ng build` sin errores; `src/` ya **no** está vacío. El eslabón `PR/commit` (commit real en Git) y la ejecución de `PR-IMPL-003` (lógica de dominio real de `plataforma`) siguen **pendientes**. El resto de `FSD-UC-NNN` permanece sin `DD-UC-NNN` propio.
...
| §10 Prompt mapping | sí (crece con `PR-IMPL-*`) | `docs/PROMPT_MAPPING.md` v2.4 (área `IMPL`, filas `PR-IMPL-001`/`PR-IMPL-002`/`PR-IMPL-003`, las tres en `docs/prompts/impl/`) |
...
- [x] §A.1 Changelog de implementación poblado y al día (9 filas: apertura de capa viva + delta de stack + generalización del dominio + multi-rol + bootstrap `DD-UC-001`/`PR-IMPL-001` + módulo `identidad` `DD-UC-002`/`PR-IMPL-002` + módulo `plataforma` `DD-UC-003`/`PR-IMPL-003` + ejecución real de `PR-IMPL-001` + **ejecución real de `PR-IMPL-002` con `ADR-0012` aplicado**).
...
- [x] §A.3 Estado por FSD-UC con su Design Doc (`FSD-UC-011` en `en progreso` con `DD-UC-003`, aún solo bootstrap; `FSD-UC-021` en `en progreso` con `DD-UC-002`, **con lógica de dominio real y 27/27 tests verdes**; el resto sigue `pendiente`, sin `DD-UC-NNN` aún).
- [ ] §A.4 Trazabilidad código ↔ DTP reconstruible para cada feature — **parcial**: cadena completa hasta `Design Doc`/`Prompt`/`ADR`; `PR-IMPL-001` y `PR-IMPL-002` ya ejecutados y verificados (`ModularityTests` verde, `ng build` verde, 27/27 tests, smoke test de Swagger/OpenAPI); falta el eslabón `PR/commit` (commit real en Git) y la ejecución de `PR-IMPL-003` (lógica de dominio real de `plataforma`).
...
- [x] `docs/PROMPT_MAPPING.md` ampliado con prompts de implementación (`PR-IMPL-*`) — filas `PR-IMPL-001`/`PR-IMPL-002`/`PR-IMPL-003` (v2.4), los tres materializados en `docs/prompts/impl/`.
