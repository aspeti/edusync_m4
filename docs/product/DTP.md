...
version: v1.6                  # versiona la implementación, no el diseño de M4
fecha: "28/05/2026"
...
| `FSD-UC-011` (Gestión de Tenants y Suscripciones) | `DD-UC-003` | en progreso | `release/3.0.0` | — | `DD-UC-003` (módulo `plataforma`: `Tenant`, ciclo de suscripción, scheduler de vencimiento, `TenantConsultaPort`) creado y aprobado; `PR-IMPL-003` generado, ejecución de código pendiente. Consume `UsuarioCreacionPort` de `identidad` (`DD-UC-002`) para crear el primer `ADMIN` del tenant en una llamada REST separada. Diseño del tenant "demo" diferido a un Design Doc de seguimiento aún sin crear |
...
- [x] §A.1 Changelog de implementación poblado y al día (7 filas: apertura de capa viva + delta de stack + generalización del dominio + multi-rol + bootstrap `DD-UC-001`/`PR-IMPL-001` + módulo `identidad` `DD-UC-002`/`PR-IMPL-002` + módulo `plataforma` `DD-UC-003`/`PR-IMPL-003`).
...
- [x] §A.3 Estado por FSD-UC con su Design Doc (`FSD-UC-011` en `en progreso` con `DD-UC-003`; `FSD-UC-021` en `en progreso` con `DD-UC-002`; el resto sigue `pendiente`, sin `DD-UC-NNN` aún).
- [ ] §A.4 Trazabilidad código ↔ DTP reconstruible para cada feature — **pendiente**: cadena completa hasta `Design Doc`/`Prompt`/`ADR`; falta el eslabón `PR/commit` (ejecución de `PR-IMPL-001`, `PR-IMPL-002` y `PR-IMPL-003`).
