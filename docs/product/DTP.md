...
version: v1.7                  # versiona la implementación, no el diseño de M4
fecha: "28/05/2026"
...
| `FSD-UC-021` (Usuarios y Roles / login) | `DD-UC-002` | en progreso | `release/3.0.0` | `ModularityTests`: 7/7 verde (solo bootstrap, sin lógica de `Usuario` todavía) | `DD-UC-002` (módulo `identidad`: `Usuario`/`UsuarioRol`, login JWT, seed `SYSADMIN`, `TenantContextProvider` real) creado y aprobado; `PR-IMPL-002` generado, **ejecución de código de dominio pendiente** (solo existe el paquete vacío `identidad/` tras `PR-IMPL-001`). Cubre solo la autenticación (parcial de `FSD-UC-021`); el CRUD administrativo completo (alta desde Admin, `PATCH roles`, activar/desactivar, restablecer contraseña) es `DD-UC-004`, todavía sin crear |
...
Estado actual: la cadena existe completa hasta `Design Doc` para `FSD-UC-011`/`FSD-UC-021` → `docs/product/FSD.md` → `docs/design/{DD-UC-001,DD-UC-002,DD-UC-003}.md` → `docs/prompts/impl/{PR-IMPL-001,PR-IMPL-002,PR-IMPL-003}.md` (registrados en `docs/PROMPT_MAPPING.md` v2.4) → `ADR-0011`/`ADR-0010`/`ADR-0009`/`ADR-0001`. **`PR-IMPL-001` ya se ejecutó** (18/07/2026): `backend/`, `frontend/` e `infra/docker-compose.yml` existen con el esqueleto de bootstrap (`Tests/Evals` = `ModularityTests` 7/7 verde, `ng build` sin errores); `src/` ya **no** está vacío. El eslabón `PR/commit` (commit real en Git) y la ejecución de `PR-IMPL-002`/`PR-IMPL-003` (lógica de dominio real de `identidad`/`plataforma`) siguen **pendientes**. El resto de `FSD-UC-NNN` permanece sin `DD-UC-NNN` propio.
...
- [x] §A.1 Changelog de implementación poblado y al día (8 filas: apertura de capa viva + delta de stack + generalización del dominio + multi-rol + bootstrap `DD-UC-001`/`PR-IMPL-001` + módulo `identidad` `DD-UC-002`/`PR-IMPL-002` + módulo `plataforma` `DD-UC-003`/`PR-IMPL-003` + **ejecución real de `PR-IMPL-001`**).
- [x] §A.2 Deltas vs DTI vFinal, **cada uno con ADR** (4 deltas → `ADR-0008` stack, `ADR-0009` generalización del modelo de dominio, `ADR-0010` multi-rol + `SysAdmin` sin tenant permanente, `ADR-0011` monolito modular + paquete `com.edusync`). `DD-UC-002` y `DD-UC-003` no añaden delta nuevo (sus decisiones de §2/§3 son de bajo riesgo, documentadas en el propio Design Doc, sin ADR dedicado).
- [x] §A.3 Estado por FSD-UC con su Design Doc (`FSD-UC-011` en `en progreso` con `DD-UC-003`; `FSD-UC-021` en `en progreso` con `DD-UC-002`; ambos con `Tests/Evals` poblado tras la ejecución de `PR-IMPL-001`; el resto sigue `pendiente`, sin `DD-UC-NNN` aún).
- [ ] §A.4 Trazabilidad código ↔ DTP reconstruible para cada feature — **parcial**: cadena completa hasta `Design Doc`/`Prompt`/`ADR`, y el esqueleto de `PR-IMPL-001` ya ejecutado y verificado (`ModularityTests` verde, `ng build` verde); falta el eslabón `PR/commit` (commit real en Git) y la ejecución de `PR-IMPL-002`/`PR-IMPL-003` (lógica de dominio real).
